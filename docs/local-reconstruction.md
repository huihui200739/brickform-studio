...
在线网站不能直接调用另一台电脑的 GPU。无需账号的重建只在这台电脑的本机工作台运行；可把生成的 GLB 下载后导入在线网站。原始 GLB 不带照片纹理。V12 会在本机重建完成后，用去背景参考图自动恢复可见区域的积木配色；背面颜色为邻近区域推测。导入 GLB 后也可上传对应参考图并点击“按参考图恢复配色”。原始 GLB 下载不包含该配色；积木模型、清单和说明书包含配色。
