...
export function manualHTML(model: Model, range?: ManualRange) {
  if (model.assembly) return assemblyManual(model, range);
...
function assemblyManual(model: Model, range?: ManualRange) {
  ...
  const start = Math.max(0, range?.start ?? 0),
    end = Math.min(a.steps.length, range?.end ?? a.steps.length);
  const pages = a.steps
    .flatMap((s, i) => {
      ...
      const batch = stageBricks(model, i),
        pages: string[] = [];
      pages.push(
        `<section class="page"><header>BRICKFORM / 本组总览 <span>${esc(model.name)}</span></header><h2>${String(i + 1).padStart(2, '0')} · ${esc(cleanStageName(s.name))}</h2><p>本组共 ${batch.length} 块，先备好下列零件，再按下一页逐块安装。图中数字对应本组的安装顺序。</p><div class="diagram group-map">${topDiagram(model, i, batch.length - 1, true)}</div><p class="group-intro">${model.imageDesign || model.meshDesign ? '定位图中，字母从左向右，数字向上增大。' : '↑ 前方是鸭嘴方向。字母从左向右，数字从后向前。'}定位格取零件的左后角；侧装件请看后面的局部图。</p><table><thead><tr><th>零件</th><th>颜色 / 编号</th><th>准备数量</th></tr></thead><tbody>${inventory(
          batch,
        )
          .map(
            (p) =>
